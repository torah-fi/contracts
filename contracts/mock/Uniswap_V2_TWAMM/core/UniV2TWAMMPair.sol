...
contract UniV2TWAMMPair is IUniswapV2PairPartialV5, UniV2TWAMMERC20 {
    ...
    function transferAmount0In(uint256 amount0In) internal returns (uint256) {
        // prev balance
        uint256 bal0 = IERC20V5(token0).balanceOf(address(this));
    ...
    function transferAmount1In(uint256 amount1In) internal returns (uint256) {
        // prev balance
        uint256 bal1 = IERC20V5(token1).balanceOf(address(this));
    ...
    function longTermSwapFrom0To1(uint256 amount0In, uint256 numberOfTimeIntervals)
        external
        lock
        onlyWhitelist
        isNotPaused
    ...
    {
        uint256 amount0 = transferAmount0In(amount0In);
    ...
    ///@param numberOfTimeIntervals number of time intervals over which to execute long term order
    function longTermSwapFrom1To0(uint256 amount1In, uint256 numberOfTimeIntervals)
        external
        lock
        onlyWhitelist
        isNotPaused
        ...
        returns (uint256 orderId)
    {
        uint256 amount1 = transferAmount1In(amount1In);
    ...
    function getTwammSalesRateEnding(uint256 _blockTimestamp)
        public
        view
        returns (uint256 orderPool0SalesRateEnding, uint256 orderPool1SalesRateEnding)
    {
    ...
    function getTwammRewardFactor(uint256 _blockTimestamp)
        public
        view
        returns (uint256 rewardFactorPool0AtBlock, uint256 rewardFactorPool1AtBlock)
    {
        uint256 lastExpiryBlock = _blockTimestamp - (_blockTimestamp % longTermOrders.orderTimeInterval);
    ...
    function getTwammOrderProceeds(uint256 orderId, uint256 blockTimestamp)
        public
        view
        returns (bool orderExpired, uint256 totalReward)
    {
        ...
        LongTermOrdersLib.OrderPool storage orderPool = LongTermOrdersLib.getOrderPool(
            longTermOrders,
            longTermOrders.orderMap[orderId].sellTokenId
        );
